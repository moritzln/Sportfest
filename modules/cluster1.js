...
function cluster_size(clusters){
	...
	while(ideal < 1){
		...
		for(var c in clusters){
			...
			if(cl.length > max_cluster_length){
				...
				cl.length = clusters[c].students.length;
				pushCluster(new_students.length, new_students)
			}else if(cl.length < min_cluster_length){
				var newLength = mergeCluster.length+cl.length;
				if(newLength <= max_cluster_length){
					mergeCluster.students = mergeCluster.students.concat(cl.students);
					mergeCluster.length = mergeCluster.students.length;
					clusters.splice(c,1);
					ideal -= 0.5;
				}else{
					mergeCluster.students = mergeCluster.students.concat(cl.students.slice(0,max_cluster_length-mergeCluster.length-1));
					cl.students.splice(0,max_cluster_length-mergeCluster.length-1);
			...
			if(mergeCluster.length > min_cluster_length){
				pushCluster(mergeCluster.students.length, mergeCluster.students);
				mergeCluster = {
					length: 0,
					students: []
				}
	...
	for(var c in clusters){
		...
		clusters[c].evaluation = global.store.tournament.evaluateCluster(clusters[c]);
	}
...
global.getDistance = function(student1, student2){
	...
	distances.push(genderDist*weight.gender);
